...
import {
  ...
  Ruler, Weight, Gauge, Plus, Minus as MinusIcon, Rocket,
} from 'lucide-react';
...
function MomentumHeadline() {
  ...
  // Sessions this week only — the this-vs-last comparison lives in WeeklyReview.
  const thisSessions = useMemo(() => {
    const thisStart = startOfWeek(new Date()).getTime();
    const count = (dates: string[]) =>
      dates.reduce((a, d) => (new Date(d).getTime() >= thisStart ? a + 1 : a), 0);
    return (
      count(workouts.map((w) => w.date)) +
      count(bwWorkouts.map((w) => w.date)) +
      count(cardio.map((c) => c.date))
    );
  ...
  return (
    <div className="card-elevated card-luxury-border rounded-2xl p-4 space-y-3 relative">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="label-cap-strong text-forge-text flex-1">{t('stats.momentum.weeklyGoal')}</span>
      ...
      <div className="text-[10px] font-mono text-forge-muted text-center">
        {goalMet
          ? t('stats.momentum.goalMet', { count: thisSessions })
          : t('stats.momentum.goalProgress', { done: thisSessions, goal })}
      </div>
    </div>
  );
}
